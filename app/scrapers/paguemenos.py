...
class PagueMenosScraper(BaseScraper):
    ...
    def _parse_product(self, product: dict, requested_ean: Optional[str] = None) -> PriceQuote:
        ...
        # Se foi solicitado um EAN, tenta achar o item com o EAN correspondente
        ...
        async with self.get_http_client() as client:
            try:
                ...
                if response.status_code == 200:
                    ...
                    if products:
                        # Se pesquisamos por EAN, verifica se algum dos produtos tem o EAN desejado
                        for prod in products:
                            item_eans = [it.get("ean") for it in prod.get("items", [])]
                            if ean in item_eans:
                                return self._parse_product(prod, requested_ean=ean)
                        
                        # Se não achou exato pelo item, mas retornou apenas 1 produto
